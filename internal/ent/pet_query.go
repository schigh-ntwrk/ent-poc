...
func (pq *PetQuery) QueryOwner() *CustomerQuery {
...
func (pq *PetQuery) QueryAppointments() *AppointmentQuery {
...
func (pq *PetQuery) WithOwner(opts ...func(*CustomerQuery)) *PetQuery {
	query := &CustomerQuery{config: pq.config}
...
func (pq *PetQuery) WithAppointments(opts ...func(*AppointmentQuery)) *PetQuery {
	query := &AppointmentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PetQuery) sqlAll(ctx context.Context) ([]*Pet, error) {
	...
	if query := pq.withAppointments; query != nil {
		...
		query.Where(predicate.Appointment(func(s *sql.Selector) {
			s.Where(sql.InValues(pet.AppointmentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.pet_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "pet_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "pet_id" returned %v for node %v`, *fk, n.ID)
			}
