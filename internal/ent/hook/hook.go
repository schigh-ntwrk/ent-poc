...
// The VeterinarianFunc type is an adapter to allow the use of ordinary
// function as Veterinarian mutator.
type VeterinarianFunc func(context.Context, *ent.VeterinarianMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VeterinarianFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.VeterinarianMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VeterinarianMutation", m)
	}
	return f(ctx, mv)
}
